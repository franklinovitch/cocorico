...
module Vagrant
  module AutoConfigure
    ...
    def self.define_docker(config, name, params)
      ...
      script_path = self.is_main_project ? "deployment/ansible.sh" : File.join(ENV["cocorico_CLOUD_HOME"], "deployment/ansible.sh")
      ...
      config.vm.define name, autostart: false do |config|
        ...
        config.vm.provider :docker do |d|
          d.image = "cocorico:latest"
          d.name = "cocorico"
          d.cmd = ["/usr/local/bin/cocorico-run.sh"]
